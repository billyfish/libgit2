...
{
	amiga_close_lib (SocketBase, ISocket);
}
...
static int amiga_open_device (struct Device **device_pp, const char *device_name_s, const uint32_t device_version, struct Interface **interface_pp, const char * const interface_name_s, const uint32_t interface_version)
{
	int ret = -1;
	if ((*library_pp = IExec->OpenDevice (device_name_s, device_version)) != NULL) {
		if ((*interface_pp = IExec->GetInterface (*library_pp, interface_name_s, interface_version, NULL)) != NULL) {
			ret = 0;
		} else {
			IExec->CloseLibrary (*library_pp);
